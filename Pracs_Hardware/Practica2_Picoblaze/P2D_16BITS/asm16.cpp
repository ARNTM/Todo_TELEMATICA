...
#define MAX_LINE_COUNT 65536 /* max 1000 lines allowed */            // Máximo numero de lineas del programa, incluye lineas con comentarios.
...
#define REG_COUNT 16			/* max 8 namereg can be declared */     // Maximo Numero de registros 
...
int register_number(char *s)
{
	...
	if(strlen(s) == 2){
		if((*(s+1) >= '0') && (*(s+1) <= '9')) return (*(s+1) - '0');	// *(s+1) - '0' sirve para convertir a un numero. *(s+1) sirve para saltarnos la S del registro S1
		else return( -1 );
	}
	if(strlen(s) == 3){	
...
void insert_instruction(char *s, int p)                                    // crea codigo máquina: campo de instrucciones
{
	...
	program_word[p] = program_word[p] | (n << 20);
}
...
{
	program_word[p] = program_word[p] | (unsigned) (c << 17);
}
	...
	for(i = 0; i < line_count; i++){                                      // para todas las lineas del programa
		if(op[i].instruction != NULL){                                    // si la linea contiene codigo de operación comprueba si conincide con las definidas en el juego de instrucciones
			...
			switch (j)                                                    // comprueba a que clase de codigo de operación pertenece
			{
				...
				case 30: /* SWAP */ /* added new instruction, same syntax with shift/rotate */
					if(op[i].op2 != NULL){
