...
public class Quiz4 implements IAlgorithm
{
	...
	public String executeAlgorithm(NetPlan netPlan, Map<String, String> algorithmParameters, Map<String, String> net2planParameters)
	{
		...
		op.addDecisionVariable("xx_p" , isNonBifurcated , new int [] {1 , netPlan.getNumberOfRoutes () } , 0 , 1);  // fraction between 0 and 1

		/* Set the objective function */
		op.setInputParameter("l_p" , netPlan.getVectorRouteNumberOfLinks() , "row");
		op.setInputParameter("h_p" , netPlan.getVectorRouteOfferedTrafficOfAssociatedDemand() , "row");
		op.setObjectiveFunction("minimize" , "sum (l_p .* h_p .* xx_p)"); 
		...
		for (Demand d : netPlan.getDemands ())
		{
			...
			op.addConstraint ("sum(xx_p(P_d)) == 1");
		}
		...
		for (Link e : netPlan.getLinks ())
		{
			...
			op.addConstraint ("sum(h_p(P_e) .* xx_p(P_e)) <= u_e");
		}
		...
		final double [] xx_p = op.getPrimalSolution("xx_p").to1DArray();
		
		for (Route r : netPlan.getRoutes())
		{
			final double h_p = r.getDemand().getOfferedTraffic();
			r.setCarriedTraffic(h_p * xx_p [r.getIndex ()] , h_p * xx_p [r.getIndex ()]);
		}
